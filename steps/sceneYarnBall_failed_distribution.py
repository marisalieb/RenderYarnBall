...
def main(
    ...
    for i in range(num_tori2):
        ...
        θ = golden_angle * i
        z = 1 - 2*i/(num_tori2 - 1)           # from +1 to -1
        r = math.sqrt(1 - z*z)
        x = r * math.cos(θ)
        y = r * math.sin(θ)
        # now (x,y,z) is a point on the unit sphere

        # # use that as a “look-at” target or axis for your torus
        # ri.TransformBegin()
        # ri.Translate(0, 0, 0)
        # # Rotate the torus so its “hole axis” points to (x,y,z)
        # ri.Rotate(90, 1, 0, 0)             # bring the donut into “equator”
        # ri.Rotate(math.degrees(math.acos(z)), 1, 0, 0)  # tilt down by arccos(z)
        # ri.Rotate(math.degrees(math.atan2(y, x)), 0, 0, 1)  # spin around Z
        # ri.Scale(0.125,0.125,0.125)
        # ri.Scale(.40510,.40510,.40510)
        # ri.Torus(rmaj_values[i], tube_radius, 0, 360, 360)
        # ri.TransformEnd()
        ...
        Rmaj3 = Rmaj2 + i * tube_radius * 2 * 0.9 * noise
        ri.Torus(Rmaj3, tube_radius, 0, 360, 360)

        # Rmaj2 = random.uniform(1.1, 1.3)
        # #Rmaj2 = float(rmaj_values[i])
        # #ri.Torus(.325, 0.05, 0.0, 360.0, 360.0)
