...
def main(
    ...
    ri.Displace("PxrDisplace", "floorDisplacement", {
        ...
        "float dispAmount": [.05],  # Adjust depth of displacement
    })
    ...
    for i in range(num_tori):
        ...
        # # 1) compute a radius that goes from R_min up to R_max
        # t = i / float(num_tori-1)           # 0.0 → 1.0
        # Rmaj = R_min + (R_max - R_min) * t  # linearly ramped major radius

        # # 2) pick a random orientation
        # rx, ry, rz = (random.random()*360 for _ in range(3))

        # # 3) optional: pick a random “push‐out” distance 
        # #    here we simply stick every torus out at its own radius:
        # tx = Rmaj * math.sin(random.random()*math.pi)
        # ty = Rmaj * math.cos(random.random()*math.pi)
        # tz = Rmaj * math.sin(random.random()*math.pi)
        ...
        ri.Bxdf("PxrSurface", "yarnShader",
        {
            ...
            "color diffuseColor" : [1.0, 0.075, 0.02],  # Slightly warmer diffuse
        ...
        Rmaj = random.uniform(10.0, 13.0)
        #ri.Torus(.325, 0.05, 0.0, 360.0, 360.0)
        ri.Scale(.040510, .040510, .040510)
        ri.Torus(Rmaj, 0.24181251, 0, 360, 360)
        #wobbly_torus_uvs(ri)
        ...
        effective_major = Rmaj * 0.040510  # 13.251 * 0.040510
...
if __name__ == "__main__":
    ...
    pixelvar = args.pixelvar if args.pixelvar is not None else 0.001
