...
#         jitter = 1.5 # 0 is zigziggy, higher is slightly less; Randomness to introduce slight variation in curl direction
#         # curl_strength = random.uniform(1.5, 3.0)  # Control how tightly the hair curls
        
#         # # Increased curl factor for tighter curls
#         # curl_factor = random.uniform(15.0, 60.0)  # Higher frequency for tight, 4C curls
...
#         variation = random.uniform(0.5, 2.5)  # Vary hair length -50% + 150%
...
#             # curl_z = math.sin(curl_factor * t + angle_offset) * 0.6  # Smaller curls in Z
...
#             # Normalize the new direction vector, makes it a better length
...
# def generate_hair(pts, widths, npts, count=900, major_radius=1.0, 
...
#             t = i / (num_control_points - 1) # range of the points in the hair strand, 1/10, 2/10 etc
...
#     jitter = .5  # used for curliness

#     # loop over each hair root
#     for (x, y, z), (nx, ny, nz) in zip(surface_pts, surface_norms):
#         # Randomize base direction slightly
#         base_dx = nx + random.uniform(-jitter, jitter)
#         base_dy = ny + random.uniform(-jitter, jitter)
#         base_dz = nz + random.uniform(-jitter, jitter)

#         # Normalize base direction
#         length = math.sqrt(base_dx ** 2 + base_dy ** 2 + base_dz ** 2)
#         base_dx /= length
#         base_dy /= length
#         base_dz /= length
...
#             # Add fresh jitter per point, but DO NOT accumulate
#             dx = base_dx + random.uniform(-jitter, jitter)
#             dy = base_dy + random.uniform(-jitter, jitter)
#             dz = base_dz + random.uniform(-jitter, jitter)
...
#             # Generate the point along the jittered direction
#             px = x + dx * strand_length * t
#             py = y + dy * strand_length * t
#             pz = z + dz * strand_length * t
#             pts.extend([px, py, pz])
...
def main(
    ...
    ri.Attribute("visibility", {"camera": [0]})  
    ...
    for i in range(num_tori2):
        ...
        ri.Attribute("displacementbound", {"float sphere": [0.1]}) # .2
        ri.Attribute("dice", {
            "float micropolygonlength": [0.1]
        })
        ...
        # # !!!!
        generate_hair(hair_pts, hair_widths, hair_npts, count=3300, major_radius=effective_major, minor_radius=effective_minor, hair_length=hairlength, hair_width=hairwidth)
