...
def wobbly_torus(ri, major_radius=13.251, minor_radius=0.34181251, u_steps=6, v_steps=300, wobble_freq=0, wobble_amp=0.0):
    points = []
    normals = []
    ...
    for i in range(u_steps):
        for j in range(v_steps):
            ...
            nverts.append(4)
            verts += [i0, i1, i2, i3]
    print(f"Generating torus: {u_steps} u-steps, {v_steps} v-steps -> {len(points)} points")
...
#         "catmull-clark",
#         nverts,       # same [4,4,4…]
#         verts,        # same connectivity
#         [],           # no extra “parameter declarations” here
#         [],           # no extra data arrays
#         {
#         "P":  points_flat,
#         "st": st_flat
#         }
...
def wobbly_torus_uvs(ri,
                     ...
                     u_steps=64, v_steps=32,
                     wobble_freq=3, wobble_amp=0.5):
    # 1) Build positions + per-vertex u/v
    points = []
    u_flat = []
    v_flat = []
    ...
    for u in u_range:
        su = u / (2*math.pi)   # normalized u in [0,1)
        Rw = major_radius + math.sin(u*wobble_freq) * wobble_amp
        for v in v_range:
            tv = v / (2*math.pi)  # normalized v
    ...
    # 3) Emit with u and v as separate primvars
    ri.PointsPolygons(
        nverts,
        verts,
        {
          "P": points_flat,
          "u": u_flat,      # now your shader’s `u` sees this
          "v": v_flat       # and its `v` sees this
        }
    )
...
def wobbly_torus_displaced(ri,
                           major_radius=13.25, minor_radius=0.34,
                           u_steps=128, v_steps=5,
                           wobble_freq=5, wobble_amp=0.5,
                           subdiv=4):  # <-- new parameter

    # bump up your effective resolution
    us = [2*math.pi * i/(u_steps*subdiv) for i in range(u_steps*subdiv + 1)]
    vs = [2*math.pi * j/(v_steps*subdiv) for j in range(v_steps*subdiv + 1)]
    ...
    # flatten
    P_flat  = [c for p in P  for c in p]
    ST_flat = ST

    # connectivity over the (u_steps*subdiv+1)x(v_steps*subdiv+1) grid
    nverts = []
    verts  = []
    rowlen = v_steps*subdiv + 1
    faces  = u_steps*subdiv * v_steps*subdiv
    for i in range(u_steps*subdiv):
        for j in range(v_steps*subdiv):
            i0 = i*rowlen + j
            i1 = (i+1)*rowlen + j
            i2 = (i+1)*rowlen + (j+1)
            i3 = i*rowlen     + (j+1)
...
def main(
    ...
    for i in range(num_tori):
        ...
        ri.Attribute("displacementbound", {"float sphere": [0.15]}) # .2
        ri.Attribute("dice", {
        ...
        # )
        ri.Pattern(
            "disp_wobbly", "disp_wobbly",
            {
                "float scale1": [.10],  # Larger displacement for the first spiral
                "float repeatU1": [2.5],  # Repeat factor for the first spiral
                "float repeatV1": [.50],  # Repeat factor for the first spiral

                "float scale2": [0.00],  # 0.04
                "float repeatU2": [150],  # 
                "float repeatV2": [-37],  # -7 
        ...
        ri.Displace(
            "PxrDisplace", "pxrdisp",
            {"reference float dispScalar": ["disp_wobbly:resultF"]}
        )
        ...
        ri.Pattern(
            "spiralColour", "spiralColour",
            {
                ...
                "float repeatU": [.5],  # Repeat factor for the first spiral
                "float repeatV": [.5],  # Repeat factor for the first spiral
                #"float blendSharpness" : [10],  # Blend sharpness for the color transition
                "color colorB": [0.9, 0.085, 0.08],
                "color colorA": [1, 0.95, 0.09],
