...
def main(
    ...
    ri.Display("scene.exr", "it", "rgba") # filenmae openexr in place of it , in terminal it scene.exr
    ...
    # # IMAGE TWWWWOOO NEW SIZE!!!!
    ri.Rotate(-5, 1, 0, 0)
    ri.Rotate(5, 0, 1, 0) 
    #ri.Rotate(12, 0, 0, 1) 
    ri.Translate(0.07065, .945328750, 1.075)      # Camera position (focusDistance should match if you want to focus here)
    # ri.Translate(0, 0, 0.05)
    ri.Translate(-0.025, 0, 0) # there is also the second at 0.1 more 
    ri.DepthOfField(32, .0250, .1125925)  # aktuell
    ...
    ri.Rotate(320, 0, 1, 0)         # Camera rotation; value, x, y, z
    ri.Rotate(-90, 1, 0, 0)
    ...
    ri.Translate(0, -.4925, 0)  # Slightly adjust position to avoid z-fighting



    num_tori3 = 0 #200 # 150 # 214 #214  # 213 #50 
    ...
    for i in range(num_tori3):
        ...
        ri.Pattern(
            "disp", "disp",
            {
                "float scale1": [.0079654],  # Larger displacement for the first spiral
                "float repeatU1": [100],  # Repeat factor for the first spiral
                "float repeatV1": [3.0],  # Repeat factor for the first spiral

                "float scale2": [0.00134],  # 0.04
                "float repeatU2": [250],  # 
                "float repeatV2": [-34],  # -7 

                "float noiseAmount1": [0.004215],   # Larger bumps noise strength
                "float noiseFreq1": [.750],     # Larger bumps frequency

                "float noiseAmount2": [0.00432],   # 0.02, can make it bigger maybe slightly
                "float noiseFreq2": [10.0]  ,    # between 4 and 5 

                "float noiseAmount3": [0.00065],   # maybe 0.005 or 0.002 but maybe more just slight stripes not bumps
        ...
        ri.Pattern(
            "spiralColour", "spiralColour",
            {
                ...
                "float repeatV": [-40.0],  # Repeat factor for the first spiral
        ...
        ri.Bxdf("PxrSurface", "yarnShader",
        {
            ...
            "float specularRoughness" : [0.5],  # Slightly blurrier reflections
        ...
        ri.Torus(Rmaj3, 0.034181251, 0, 360, 360) #0.024181251
        ri.TransformEnd()  # 5
        ri.AttributeEnd() # C
        ...
        hairlength = 0.0075 *hair_scale * 1.7
        hairwidth = 0.000215 * hair_scale * 1
        ...
        #     'color specularColorTRT': [1.0, 0.9, 0.9],      
        #     'color specularColorTT': [1.0, 0.9, 0.9],
    ...
    random.seed(70)  # makes your randomness repeatable across runs
    ...
    # for i, ((tx, ty, tz), (rx, ry, rz)) in enumerate(zip(manual_positions, manual_rotations)):
        ...
        ri.Pattern(
            "disp", "disp",
            {
                "float scale1": [.008579654],  # Larger displacement for the first spiral
                "float repeatU1": [100],  # Repeat factor for the first spiral
                "float repeatV1": [3.0],  # Repeat factor for the first spiral
                ...
                "float noiseAmount3": [0.0015],   # maybe 0.005 or 0.002 but maybe more just slight stripes not bumps
                "float noiseFreq3": [40.0]      # the smallest bumps on it
            }
        )
        ...
        ri.Pattern(
            "spiralSpecNoise", "spiralSpecNoise",
            {
                ...
                "float noiseFreq1": [1000.750],
                "float maskScale": [1.5],  # Use 1.5 for more matte, or 0.5 for glossier
        ...
        ri.Scale(0.125, 0.125, 0.125)  # Scale down the sphere
        ri.Scale(.40510, .40510, .40510)
        ri.Torus(Rmaj2, 0.034181251, 0, 360, 360)  # removed the 2 from in front of 4 
        #wobbly_torus_uvs(ri)
    ...
    #     "disp", "disp",
    #     {
    #         "float scale1": [.007654],  # Larger displacement for the first spiral
    #         "float repeatU1": [100],  # Repeat factor for the first spiral
    #         "float repeatV1": [3.0],  # Repeat factor for the first spiral
    ...
    # ri.Translate(0, 0.063510, 0.0)
    # ri.Translate(0, 0, -0.025)
    # ri.Translate(-0.02735, 0, 0)
    # # ri.Rotate(10, 0, 1, 0)
    # # ri.Rotate(10, 0, 0, 1)

    # ri.Scale(0.125, 0.125, 0.125)
    # ri.Scale(.40510, .40510, .40510)
    # ri.Torus(1.0, 0.04324181251, 0, 360, 100)

    # ri.AttributeEnd() # G

    # ri.TransformEnd() # 9 #???? maybe out again also now one end at the end too many
    ...
    # NEW BALL
    num_tori4 = 0# 150 # 150 # 214 #214  # 213 #50 
    rmaj_values = np.linspace(.8, 1.1, num_tori4)


    ri.Translate(0.1, 0,-0.025) # , -0.05)
    ...
    for i in range(num_tori4):
        ...
        ri.Torus(Rmaj4, 0.034181251, 0, 360, 360) #0.024181251
        # ri.TransformEnd()  # End the torus transform
        ri.AttributeEnd()
        ...
        hair_scale = .125
        # Match the effective torus radii
        effective_major = Rmaj4 * 0.40510 *.125 # 13.251 * 0.040510
        effective_minor = 0.034181251 * 0.40510 * .125
        ...
        #     'float diffuseGain': [0.1],                     
        #     # 'color diffuseColor': [0.85, 0.75, 0.6],  
        #     'color diffuseColor': [0.0, 01.0, 01.0],  
    ...
    # for i, ((tx, ty, tz), (rx, ry, rz)) in enumerate(zip(manual_positions, manual_rotations)):
     ...
     # colour spiral pattern to simulate strands of fibres
        ...
        ri.Scale(0.125, 0.125, 0.125)  # Scale down the sphere
        ri.Scale(.40510, .40510, .40510)
        ri.Torus(Rmaj5, 0.034181251, 0, 360, 360)  # removed the 2 from in front of 4 
        #wobbly_torus_uvs(ri)

        # ri.TransformEnd()
        ri.AttributeEnd()

        # ri.TransformEnd() #??? or back one 
        ...
        hair_scale = .125
        # Match the effective torus radii
        effective_major = Rmaj5 * 0.40510 *.125 # 13.251 * 0.040510
        effective_minor = 0.034181251 * 0.40510 * .125
        hairlength = 0.0075 *hair_scale * 1.5
        hairwidth = 0.000215 * hair_scale * 1.5
    ...
    #     "disp", "disp",
    #     {
    #         "float scale1": [.00495],  # Larger displacement for the first spiral
    #         "float repeatU1": [170],  # Repeat factor for the first spiral
    #         "float repeatV1": [8.0],  # Repeat factor for the first spiral
    ...
    # # working but tilted forward
    # # ri.Rotate(90, 1, 1, 0)
    # # ri.Rotate(20, 0, 0, 1)
    # # ri.Translate(0.0875, 0.0510, -0.005)
    # # ri.Rotate(20, 0, 1, 0)
    ...
    # # # ri.Translate(0.0875, 0.0510, -0.005)
    # # # ri.Rotate(20, 0, 1, 0)
    # # ri.Translate(0.1, -0.05910, -0.0000)
    ...
    # ri.Rotate(-90, 1, 0, 0)
    # ri.Rotate(0, 0, 0, 1)
    # ri.Translate(0.0, 0.0, -0.05)
    # ri.Translate(0.05, 0.0, -0.0)
    # ri.Translate(0.0, -0.1, -0.0)
    # ri.Rotate(10, 1, 0, 0)
    # ri.Rotate(10, 0, 1, 0)

    # # ri.Rotate(-10, 0, 0, 1)
    # # ri.Rotate(-5, 0, 1, 0)
    # # ri.Translate(0.0, 0.025, 0.0)
    # # # ri.Translate(0.0, 0.0, 0.025)
    ...
    # ri.Scale(0.125, 0.125, 0.125)
    # ri.Scale(.40510, .40510, .40510)
    # ri.Torus(2, 0.045324181251, 0, 360, 120)
