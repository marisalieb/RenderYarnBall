...
def main(
    ...
    ri.Translate(0, 0, 0)
        ...
        # Set the camera parameters with shallow depth of field (small f-stop value)
    focal_distance = 2.5  # Distance to the object in focus
    ri.Projection(ri.PERSPECTIVE, {ri.FOV: 45.0})
    ri.FStop(2.8)  # Shallow depth of field: blurry background and foreground
    ri.FocalDistance(focal_distance)
    ri.Shutter(0.0, 1.0)
    
    ri.TransformEnd()

    # Place the object in focus (e.g., at (0, 0.75, 2.5))
    ri.TransformBegin()
    ri.Translate(0, 0.75, 2.5)
    ri.Sphere(0.1, -0.1, 0.1, 360)  # Example: placing a sphere
    ri.TransformEnd()

    # Other objects placed further away, blurred out of focus
    ri.TransformBegin()
    ri.Translate(0, 2, 4)  # Object farther away
    ri.Cube(-1, 1, -1, 1, -1, 1)
    ri.TransformEnd()
    ...
    ri.Translate(0, -0.75, -2)      # Camera position (focusDistance should match if you want to focus here)
