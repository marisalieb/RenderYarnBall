...
# shader_name = "colorByPosition"  # Assuming your shader is called colorByPosition.osl
# checkAndCompileShader(shader_name)
...
ri.Pattern(
    "check", "check",
    {
        "color baseColour":   [0, 0, 0],
        "color stripeColour": [1, 1, 1],
        "float repeatU":      [18.0],    # more winds around
        "float repeatV":      [4.0],    # climbs twice up the torus
        "float width":        [0.1]    # narrower stripe
    }
)

# Apply the shader to the torus
ri.Bxdf(
    "PxrSurface", "plastic", {
...
# # Declare the OSL pattern for displacement
# ri.Pattern(
#     "spiral_displace", "spiral_displace",
#     {
#         "float repeatU": [18.0],
#         "float repeatV": [4.0],
#         "float scale":   [5],  # height of displacement
#     }
# )

# # Apply displacement using a built-in displacement shader
# ri.Displace(
#     "PxrDisplace", "MyDisplace", {
#         "reference float dispAmount": ["spiral_displace:disp"]
#     }
# )

# # Basic color shader (no stripes here, just flat color)
# ri.Bxdf(
#     "PxrSurface", "plastic", {
